...
void task_get_adc() {
    ...
    v_batt = convertTo(adc_battery, 'V');
    dist = convertTo(adc_distance, 'M'); 
}


void task_send_battery_voltage() {
    char str[16];
    sprintf(str, "$MBATT,%.2f*\n", v_batt);
...
int main() {
    ...
    IEC0bits.U1TXIE = 1; // Enable UART transmitter interrupt
