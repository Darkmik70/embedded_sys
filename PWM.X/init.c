 ...
 * @param type type ('A' accelerometer, 'G' gyroscope, 'M' magnetometer)
 * @return None
 */
void setACTIVEmode(char type)
{
    switch (type)
    {
    case 'A': // accelerometer case
        break;
    case 'G': // gyroscope case
        break;

    case 'M': // magnetometer case
        CS3 = 0;
        // Pass to SLEEP mode
        spi_write(0x4B); // register where the power bit is
