...
void __attribute__((__interrupt__, __auto_psv__))_INT1Interrupt(void) {
    ...
    // This is insufficient, we should also check if button was pressed
    if (is_pwm_on)
    {
        LATAbits.LATA0 = 1;
        //drive_right();
...
the buggy move right. When the same button E8 is pressed
again, stop generating the PWM signal.
*/
int main()
{
    is_pwm_on = 0; // Flag - 1: PWM is generated 0: NO
